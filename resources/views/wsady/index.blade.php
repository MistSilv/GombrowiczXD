<x-layout>
    ...
    document.getElementById('dodaj-produkt').addEventListener('click', () => {
        ...
        produkty.forEach(p => options += `<option value="${p.id}">${p.tw_nazwa}</option>`);
    ...
    function dodajProduktDoListy(produktId, nazwaProduktu, ilosc = 1) {
        const container = document.getElementById('produkty-lista');
        const istniejący = [...container.querySelectorAll('select')].find(s => s.value == produktId);
        ...
        if (istniejący) {
            ...
            let options = '<option value="">-- wybierz produkt --</option>';
            produkty.forEach(p => options += `<option value="${p.id}" ${p.id == produktId ? 'selected' : ''}>${p.tw_nazwa}</option>`);

            newItem.innerHTML = `
                <select name="produkty[${index}][produkt_id]" class="form-select w-full" required>
                    ${options}
                </select>
                <input type="number" name="produkty[${index}][ilosc]" min="1" value="${ilosc}" class="form-input w-24 text-black" required>
                <button type="button" class="bg-red-600 text-white rounded px-3 py-1 hover:bg-red-700 transition remove-item">✕</button>
            `;
            container.appendChild(newItem);
            index++;
